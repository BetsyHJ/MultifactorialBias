...
def get_pos_pop_bias(MCAR, trainset):
...
def get_pos_pop_bias_updated(MCAR, trainset, clip=(1e-3, 0.2), alpha=(5,5)): #clip(1e-3, 0.2) for pop-pos on all users. And (1e-3, 0.08) for only testusers.
    ...
    for r, group in MCAR.groupby(by='rating'):
        ...
        # print("!!!!!!!! Trying smoothing on p_IY !!!!!!!!!!")
        # p_IY[:, int(r)-1] = (v + alpha1) / (n_MCAR + alpha1 * p_IY.size) # (v + alpha1) / (n_MCAR + alpha1 * p_IY.size) # no-smoothing: v * 1.0 / n_MCAR
...
is_to_save, is_to_load = False, True
if is_to_save:
...
print("************ Validation: Using average loss over %s ************ " % ('users' if valid_loss_over_users else "items")) # @here
...
def MF_optmize(trainset, validset, testset, config=None, debiasing='none', use_wandb=False):
    def evaluate_on_testset(get_pred=False):
        ...
        with torch.no_grad():
            if config.get('fine_tune_smoothing_in_MCAR', False):
                test_ratings, test_users, test_items = input_to_cuda(MCAR, device)
            ...
            if task == 'ratingPrediction':
                result.update(eval_RMSE_per_group(testset_['pred'].values.clip(1, 5), testset_))
                result['rmse'] = mean_squared_error(testset_['reward'].values, testset_['pred'].values.clip(1, 5), squared=False)
                result['mse'] = mean_squared_error(testset_['reward'].values, testset_['pred'].values.clip(1, 5))
                result['mae'] = mean_absolute_error(testset_['reward'].values, testset_['pred'].values.clip(1, 5))
                ndcgs = []
                for u, group in testset_.groupby(by='user_id'):
                    if len(group) > 1:
                        ndcgs.append(ndcg_score([group['reward'].values], [group['pred'].values], k=10))
                ...
                for u, group in testset_.groupby(by='user_id'):
                    if len(group) > 1:
                        ndcgs.append(ndcg_score([group['reward'].values], [group['pred'].values], k=10))
                result['ndcg'] = np.mean(ndcgs)
                result['nll'] = log_loss(testset_['reward'].values.astype(int), testset_['pred'].values)
    ...
    # @Here 
    # print("Using posivity bias in U-learning")
    # train_propensities_pos = torch.tensor(p_O_Y[(trainset['rating'].values - 1).astype(np.int32)],dtype=torch.float32).to(device)
    # train_propensities_pos_inversed = torch.reciprocal(train_propensities_pos)
    # train_propensities_pos_inversed /= train_propensities_pos_inversed.mean()
    # valid_propensities =  torch.tensor(p_O_Y[(validset['rating'].values - 1).astype(np.int32)],dtype=torch.float32).to(device)
    # print("************* Using P(O|Y) in validation ***************") # @Here
    ...
    for i in epoch_info:
        ...
        if do_shuffle:
            ...
            if train_propensities is not None:
                ...
                # train_propensities_pos_inversed = train_propensities_pos_inversed[shuffled_index] #@here
        # U-learning: update u related params w/ P(O|IY)
        ...
        while start < train_ratings.shape[0]:
            ...
            if train_propensities is not None:
                ...
                # inversed_prop = train_propensities_pos_inversed[start:end] # @Here
                if snips:
...
config['patience'] = 5
config['train_on_testusers_only'] = True if dataset_name != 'kuairec' else False # Kuairec dataset must has the interactions not for the small matrix
config['smoothing_heat_map'] = False
...
if config.get('smoothing_heat_map', False):
    repeat = 1
    p_O_IY = get_pos_pop_bias_updated(MCAR, pd.concat([trainset, validset], ignore_index=True), alpha=(args.alpha1, args.alpha2)) # 
else:
    if dataset_name == 'yahoo':
...
for _ in range(repeat):
    ...
    #     post_processing(model, testset)
if config.get("plot", False):
    df = pd.DataFrame(data=np.array(results_for_plot[0]), columns=['epoch', 'valid_mse_ips', 'test_mse'])
    df['repeat'] = 1
    for rid, results_for_plot_per_repeat in enumerate(results_for_plot[1:]):
        df_rid = pd.DataFrame(data=np.array(results_for_plot_per_repeat), columns=['epoch', 'valid_mse_ips', 'test_mse'])
        df_rid['repeat'] = rid + 2
        df = pd.concat([df, df_rid], ignore_index=True)
    df.to_csv("learningcurve_mf_ips_mul_ALS_%s.csv" % (dataset_name), index=False)
...
if config.get('fine_tune_smoothing_in_MCAR', False):
    print("The avg results on MCAR for", results[0].keys(), " are", s)
else:
